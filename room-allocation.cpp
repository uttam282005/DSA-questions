...
void solve() {
  ...
  rep(i, 0, n) {
    cin >> arr[i].arrival >> arr[i].deprature;
    arr[i].index = i;
  }

  sort(all(arr),
       [](const person &a, const person &b) { return a.arrival < b.arrival; });
  ...
  for (auto person : arr) {
    ...
    answer[person.index] = roomNumber;
    rooms.insert({person.deprature + 1, roomNumber});
  }

  cout << counter << endl;
  for (auto roomNumber : answer) {
    cout << roomNumber << " ";
  }
}
