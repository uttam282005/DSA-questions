...
void solve() {
  ...
  while (l < r) {
    int found = -1;
    if (!isPrime[divisors[l]] && !isSquareOfPrime[divisors[l]]) {
      found = divisors[l];
    } else if (!isPrime[divisors[r]] && !isSquareOfPrime[divisors[r]])
      found = divisors[r];

    if (found != -1) {
      int c = n / found;
      int a, b;
      for (int i = 2; i * i <= found; i++) {
        if (found % i == 0 and i != c and c != found / i and i != found / i) {
          a = i;
          b = found / i;
          cout << "YES\n";
          cout << a << " " << b << " " << c << endl;
