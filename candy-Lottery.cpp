...
db binPow(db a, ll b) { // Change b's type to ll (integer)
  if (b == 0)
    return 1.0;

  db res = binPow(a, b / 2); // Fix type to db
  res = (res * res);

  if (b % 2 == 1) // Fix floating-point modulo issue
    res *= a;
...
void solve() {
  ...
  cin >> n >> k;
  db ans = 0;
  db total_combinations = binPow(k, n);
  rep(i, 1, k + 1) {
    db max_k = binPow(i, n) - binPow(i - 1, n);
    db prob_i = 1.0 * max_k / total_combinations;
    ans += i * prob_i;
  }
