...
struct Hashing {
  ...
  vector<vector<ll>> BackWardhashValues;
  ...
  Hashing(string a) {
    ...
    BackWardhashValues.resize(primes);
    ...
    for (int i = 0; i < sz(hashPrimes); i++) {
      ...
      for (int j = n - 1; j >= 0; j--) {
        BackWardhashValues[i][j] =
            ((s[j] - 'a' + 1LL) * powersOfBase[i][n - j - 1]) % hashPrimes[i];
        BackWardhashValues[i][j] =
            (BackWardhashValues[i][j] +
             (j < n - 1 ? BackWardhashValues[i][j + 1] : 0LL)) %
            hashPrimes[i];
      }
    }
  }
  ...
  vector<ll> BackWardsubstringHash(int l, int r) {
    vector<ll> hash(primes);
    for (int i = 0; i < primes; i++) {
      ll val1 = BackWardhashValues[i][l];
      ll val2 = r < n - 1 ? BackWardhashValues[i][r + 1] : 0LL;
      hash[i] = mod_mul(mod_sub(val1, val2, hashPrimes[i]),
                        inversePowersOfBase[i][n - r - 1], hashPrimes[i]);
    }
    return hash;
  }
};
...
void solve() {
  ...
  if (n == 1 ||
      hs.substringHash(0, n - 1) == hs.BackWardsubstringHash(0, n - 1)) {
    cout << s << endl;
    return;
  }
  rep(i, 0, n) {
  ...
  for (int i = n - pos - 1; i >= pos; i--) {
    if (hs.substringHash(pos, i) == hs.BackWardsubstringHash(pos, i)) {
      end_pos = i;
      break;
    }
  }
  ...
  for (int i = pos; i < n - pos; i++) {
    if (hs.substringHash(i, n - pos - 1) ==
        hs.BackWardsubstringHash(i, n - pos - 1)) {
      pref_end_pos = i;
      break;
    }
  }
  int len_prefix_palindrome = end_pos - pos + 1;
  int len_suffix_palindrome = n - pos - 1 - pref_end_pos + 1;
  string pal;
  if (len_prefix_palindrome > len_suffix_palindrome)
    pal = s.substr(pos, end_pos - pos + 1);
  else
    pal = s.substr(pref_end_pos, n - pos - 1 - pref_end_pos + 1);
  cout << s.substr(0, pos) + pal + s.substr(n - pos, pos) << endl;
}
