...
void solve() {
  ...
  while (!widths.empty()) {
    int remaining = W; // Start with full width available
    height++;

    auto it = widths.upper_bound(remaining);
    while (it != widths.begin()) {
      --it; // Move to the largest valid element
      remaining -= *it;
      widths.erase(it);
      it = widths.upper_bound(remaining);
    }
  }
