...
ll maxhei(ll curhei, map<ll, vector<ll>> &G, ll &maxi, map<ll, bool> &visited) {
  visited[curhei] = true;
  maxi = max(maxi, curhei);

  for (int poshei : G[curhei]) {
    if (visited[poshei])
      continue;
    maxhei(poshei, G, maxi, visited);
  }
...
void solve() {
  ...
  rep(i, 0, n) {
    cin >> v[i];
    G[v[i] + i].push_back(v[i] + 2 * i);
  }
  cout << maxhei(n, G, maxlen, visited) << endl;
}
