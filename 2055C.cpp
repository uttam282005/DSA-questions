...
void solve() {
  ...
  rep(i, 0, n) rep(j, 0, m) rowsum[i] += v[i][j];
  rep(j, 0, m) rep(i, 0, n) colsum[j] += v[i][j];
  ...
  while (l <= moves.size()) {
    if (moves[l] == 'D')
      v[i][j] = target - rowsum[i], rowsum[i] += v[i][j], colsum[j] += v[i][j],
      i++;
    else
      v[i][j] = target - colsum[j], rowsum[i] += v[i][j], colsum[j] += v[i][j],
      j++;
    l++;
  }
