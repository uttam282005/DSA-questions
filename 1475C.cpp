...
void solve() {
  ...
  vi ai(k);
  vi bi(k);
  map<int, int> cnt_boys;
  map<int, int> cnt_girls;
  rep(i, 0, k) cin >> ai[i], cnt_boys[ai[i]]++;
  rep(i, 0, k) cin >> bi[i], cnt_girls[bi[i]]++;
  ll ans = 0;
  itr(cnt_boys) { ans += 1LL * it.second * (k - it.second); }
  ans /= 2;
  ll invalid_pairs = 0;
  itr(cnt_girls) { invalid_pairs += 1LL * it.second * (it.second - 1) / 2; }
