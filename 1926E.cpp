...
void solve() {
  ...
  for (int i = 1; i < g; i++) {
    prefix[i] = prefix[i - 1] + group[i];
  }

  int group_n = lower_bound(all(prefix), k) - prefix.begin();

  int first_ele = binPow(2, group_n);

  int last_ele_ind = group_n != 0 ? prefix[group_n - 1] : 0;

  int first_ind = last_ele_ind + 1;

  int group_ind = k - first_ind;

  cout << (2 * group_ind + 1) * first_ele << endl;
}
