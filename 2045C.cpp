...
void solve() {
  string s, t;
  cin >> s >> t;
  vi prfxlen(26, 300000);
  rep(i, 1, s.size()) if (prfxlen[s[i] - 'a'] == 300000) prfxlen[s[i] - 'a'] =
      i;
  ...
  int suff = -1;
  for (int i = n - 2; i >= 0; i--) {
    int currlen = prfxlen[t[i] - 'a'] + n - i;
    if (currlen < minlen and prfxlen[t[i] - 'a'] != 300000) {
      minlen = currlen;
      p = prfxlen[t[i] - 'a'] + 1;
      suff = i;
    }
