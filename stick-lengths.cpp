...
void solve() {
  ...
  vll psum = prefixSum(v);
  ll mini = LONG_LONG_MAX;
  rep(i, 1, n) {
    if (v[i] != v[i - 1]) {
      ll rightSum = psum[n - 1] - psum[i];
      ll leftSum = psum[i - 1];
      ll sumR = rightSum - (n - i - 1) * v[i];
      ll sumL = i * v[i] - leftSum;
      mini = min(mini, sumR + sumL);
    }
  }
  mini = min(mini, psum[n - 1] - v[0] - (n - 1) * v[0]);
  cout << mini << endl;
}
