...
void solve() {
  ...
  cin >> n;
  vi v(n);
  vector<vector<int>> prefix_bit_sum(n + 1, vi(31));
  rep(i, 0, n) cin >> v[i];
  rep(i, 0, n) {
    rep(j, 0, 31) prefix_bit_sum[i + 1][j] +=
        prefix_bit_sum[i][j] + ((v[i] >> j) & 1);
  }
  int q;
  cin >> q;
  while (q--) {
    ...
    while (left <= right) {
      ...
      ll and_till_mid = 0;
      int sz = mid - l + 1;
      rep(i, 0, 31) if (prefix_bit_sum[mid][i] - prefix_bit_sum[l - 1][i] == sz)
          and_till_mid += binPow(2, i);
      if (and_till_mid >= k) {
        ans = mid;
        left = mid + 1;
      } else
