...
int C(int n, int k) {
  return facto(n) * 1ll * fast_pow(facto(k), mod - 2) % mod * 1ll *
         fast_pow(facto(n - k), mod - 2) % mod;
}
...
void solve() {
  ...
  int maxElement = v[n - 1];

  int k = count(v.begin(), v.end(), maxElement - 1);
  int cntMax = count(v.begin(), v.end(), maxElement);
