...
void solve() {
  ...
  for (int i = 1; i < n; i++) {
    ...
    ll leftind = lower_bound(all(avl), left) - avl.begin() + 1;
    ll rightind = lower_bound(all(avl), right) - avl.begin() + 1;

    ll leftcnt = leftind > m ? 0 : m - leftind + 1;
    ll rightcnt = rightind > m ? 0 : m - rightind + 1;

    ll curway = (leftcnt - rightcnt) * rightcnt;
    curway += rightcnt * (rightcnt - 1);
    ways += curway;
  }
